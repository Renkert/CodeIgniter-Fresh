...
/**
*
* @package  	application\core\APP_Loader.php
* @category		Core
...
class APP_Loader extends CI_Loader
{
    ...
    public function __construct()
	{
		...
		log_message('info', "[APP_Loader] -> Initialized");
     ...
     * @return	void
     */
    public function lang($file = array(), $lang = '')
	{
        if (is_array($file)) {
    ...
    private function detect_module($class)
	{
        ...
        if (($first_slash = strpos($class, '/')) !== FALSE)
		{
			...
			//log_message('info', "++++++++++++ APP_LOADER -> detect_module -> Module: ".$module." Class: ".$class);

            // Check if module exists
            //if ($this->find_module($module) || $this->find_module( $router->fetch_module() ) ) {
